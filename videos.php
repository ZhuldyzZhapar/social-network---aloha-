...
$href = 'delete.php?deleteAlbumVid=' . $album_name;
try{
	$connect1 = new PDO("mysql:host=localhost; dbname=aloha", 'root', '');
	$connect1->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$stmt1 = $connect1->prepare("SELECT video, tag_person FROM video WHERE user_id = :user_id AND album = :album ORDER BY date_time DESC;");
...
<body style='width: 100%;'>
	...
	foreach ($album_videos as $value){
	 	$video = $value['video'];
	 	if($video === 'no video'){ // if album is empty
	 	?><div class='images'>
	 	<div class='image'>
	 	<div class='image_div'><center><br><br><p>No video</p></center></div></div></div>
	 	<?php
	 	break;
		...
		if ($resolution_x > $resolution_y){
	 	...
	 	<div class='image'>
	 	<div class='image_div'>
	 	<video class='click_video' <?php echo $size;?> src='<?php echo $src;?>'></video></div></div>
	 	</div>
	<?php } }
	?>
	...
	$(".click_video").click(function(){
		...
		<?php $video = "<script>document.writeln(href1);</script>";
		$_SESSION['album'] = $album_name;
		?>
		href = 'delete.php?deleteVid=' + href1;
	...
	<?php $_SESSION['album'] = $album_name;?>
  	$("body").append("<center>" + "<div id='window' class='window' style='display: block !important;'>" + 
  		...
  		"<p>Please choose an image of <i>'.mp4', '.avi', '.mov', '.flv', '.wmv'</i> type.</p>" + 
  		"<form action='upload.php?what=video' method='POST' enctype='multipart/form-data'>" + 
  		"<input type='file' name='file'>" + "<button type='submit' name='submit'>Upload video</button><br><br>" + 
  		"</form>" + "</div>" + "</center>" + 
  		"<div id='gray' class='gray' style='display: block !important;'> ></div>"); 
	$(".window").fadeIn(500);
	$(".gray").fadeIn(500);
	...
	$(".album_rename").click(function(){
  	$("body").append("<center>" + 
		...
		"<input class='input' type='text' name='new_album_name_vid' placeholder='New name of an album' style='width: 250px !important;'required>" + 
